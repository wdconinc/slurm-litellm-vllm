...
def client():
    ...
    if not api_base or not api_key:
        pytest.fail(
            "Environment variables OPENAI_API_BASE and OPENAI_API_KEY must be set. Source run/endpoint.env first."
        )
...
def test_basic_chat_completion(client, model_name):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "user",
                "content": "Say exactly the word 'Banana' and nothing else.",
            }
        ],
        temperature=0.01,
        max_tokens=10,
...
def test_system_prompt_adherence(client, model_name):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are a French translator. Always respond by translating the user's input to French.",
            },
...
def test_multi_turn_conversation(client, model_name):
    ...
    response = client.chat.completions.create(
        model=model_name, messages=messages, temperature=0.1, max_tokens=20
    )
