...
def test_ray_multi_node_cluster():
    ...
    print(
        f"Waiting for Slurm job {job_id} to boot 2-node Ray cluster (this may take 5-10 minutes)..."
    )
    ...
    while not os.path.exists(endpoint_file):
        ...
        if job_id not in squeue_res.stdout and (time.time() - start_time) > 30:
            pytest.fail(
                f"Job {job_id} is no longer in the queue. It likely failed to start the Ray cluster. Check vllm_{job_id}.log"
            )
