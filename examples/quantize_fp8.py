...
def main():
    parser = argparse.ArgumentParser(
        description="Quantize a model to FP8 and save to disk."
    )
    parser.add_argument(
        "--model-id",
        type=str,
        required=True,
        help="Hugging Face Model ID (e.g., sahilchachra/Leanstral-1.5-119B-A6B-BF16)",
    )
    parser.add_argument(
        "--save-path",
        type=str,
        required=True,
        help="Local directory to save the FP8 checkpoint",
    )
    ...
    print(
        "Done! You can now update config/models.yaml 'fullname' to point to this local directory."
    )
