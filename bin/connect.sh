...
echo "Fetching endpoint configuration from $HOST..."
ENDPOINT_BASE=$(ssh -q "$HOST" "cat ~/litellm/etc/endpoint.env 2>/dev/null | grep '^OPENAI_API_BASE='")

if [ -z "$ENDPOINT_BASE" ]; then
    echo "Error: Could not read OPENAI_API_BASE from ~/litellm/etc/endpoint.env on $HOST."
